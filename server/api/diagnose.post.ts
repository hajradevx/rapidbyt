...
const CHECKS: CheckDef[] = [
  {
    ...
    label: "Slow First Contentful Paint (FCP)",
    solution: "Remove render-blocking resources, inline critical CSS",
  },
  {
    key: "largest-contentful-paint",
    label: "Slow Largest Contentful Paint (LCP) — hurts SEO",
    solution: "Optimise server response time, use a CDN, preload critical resources",
  },
  {
    key: "total-blocking-time",
    label: "High Total Blocking Time — JavaScript is too heavy",
    solution: "Split and defer JavaScript bundles, remove unused scripts",
  },
  {
    key: "cumulative-layout-shift",
    label: "Layout Shift (CLS) — page elements are jumping around",
    solution:
      "Set explicit width/height on images and embeds, avoid injecting content above the fold",
  },
  {
    key: "speed-index",
    label: "Slow Speed Index — content loads too slowly",
    solution: "Reduce main-thread work and defer non-critical resources",
  },
  {
    key: "uses-optimized-images",
    label: "Images are not optimised — wasting load time",
    solution: "Compress images (e.g. Squoosh) or automate compression in your build pipeline",
  },
  {
    key: "uses-webp-images",
    label: "Not using modern image formats (WebP/AVIF)",
    solution: "Convert to WebP/AVIF — 30–80% smaller than JPEG/PNG",
  },
  {
    key: "render-blocking-resources",
    label: "Render-blocking resources slowing down page display",
    solution: "Use async/defer on scripts, inline critical CSS, remove unused styles",
  },
  {
    key: "unused-javascript",
    label: "Unused JavaScript is being loaded unnecessarily",
    solution: "Enable tree-shaking, code-split routes, remove unused dependencies",
  },
  {
    key: "unused-css-rules",
    label: "Unused CSS rules are bloating your page",
    solution: "Use PurgeCSS or your framework's built-in CSS purge to remove unused styles",
  },
  {
    key: "uses-text-compression",
    label: "Text compression (GZIP/Brotli) is not enabled",
    solution: "Enable GZIP or Brotli compression on your server/CDN",
  },
  {
    key: "uses-long-cache-ttl",
    label: "Poor caching policy — visitors re-download assets every visit",
    solution: "Set Cache-Control headers with a long TTL for static assets",
  },
  {
    key: "efficient-animated-content",
    label: "Animated GIFs instead of optimised video formats",
    solution: "Replace animated GIFs with a muted, autoplaying video (MP4/WebM)",
  },
  {
    key: "meta-description",
    label: "Missing meta description — hurts SEO click-through rate",
    solution: "Write a unique, keyword-rich meta description for every page",
  },
  {
    key: "document-title",
    label: "Missing page title tag — critical SEO issue",
    solution: "Add a unique, descriptive title tag to every page",
  },
  {
    key: "hreflang",
    label: "Missing hreflang tags for international targeting",
    solution: "Add hreflang tags if you serve multiple languages/regions",
  },
  {
    key: "http-status-code",
    label: "HTTP error status code detected",
    solution: "Fix the server so it returns a 200 OK status for this page",
  },
  {
    key: "is-crawlable",
    label: "Page is blocked from search engine crawling",
    solution: "Fix robots.txt / meta robots tags — they are currently blocking search engines",
  },
  {
    key: "link-text",
    label: 'Generic link text ("click here") hurts SEO',
    solution: "Use descriptive, keyword-relevant link text",
  },
  {
    key: "image-alt",
    label: "Images missing alt text — accessibility & SEO issue",
    solution: "Add descriptive alt attributes to all images",
  },
  {
    key: "color-contrast",
    label: "Poor colour contrast — accessibility problem",
    solution: "Increase text/background contrast to meet WCAG AA",
  },
  {
    key: "tap-targets",
    label: "Touch targets too small for mobile users",
    solution: "Ensure all buttons/links are at least 48×48px on mobile",
  },
  {
    key: "viewport",
    label: "Missing mobile viewport meta tag",
    solution: 'Add a responsive <meta name="viewport"> tag',
  },
  {
    key: "uses-https",
    label: "Site is not served over HTTPS — security risk",
    solution: "Install an SSL certificate (free via Let's Encrypt)",
  },
  {
    key: "no-vulnerable-libraries",
    label: "Vulnerable JavaScript libraries detected",
    solution: "Update all dependencies to their latest patched versions",
  },
];

function buildProblemsAndSolutions(audits: Record<string, PageSpeedAudit>): {
  problems: string[];
  solutions: string[];
} {
  ...
  for (const check of CHECKS) {
    ...
    if (audit && audit.score !== null && audit.score < 0.9) {
      ...
      const { solution } = check;
      if (solution) {
        const labelPart = check.label.split(" — ")[0] ?? check.label;
        const shortLabel = labelPart.split(" (")[0] ?? labelPart;
        solutions.push(`<strong>${escapeHtml(shortLabel)}:</strong> ${escapeHtml(solution)}`);
      }
    }
  }
...
function severityLevel(
  ...
  problemCount: number,
): { level: string; color: string; bg: string; message: string } {
  ...
  if (avg >= 85 && problemCount <= 3) {
    return {
      ...
      color: "#22c55e",
      bg: "#f0fdf4",
      message: "Your site is performing well! A few minor optimisations could push it further.",
    };
  }
  if (avg >= 60 && problemCount <= 8) {
    return {
      ...
      color: "#f59e0b",
      bg: "#fffbeb",
      message: "Your site has several fixable issues that are costing you traffic and conversions.",
    };
  }
  return {
    level: "Critical",
    ...
    bg: "#fef2f2",
    message:
      "Your site has serious performance issues that are actively losing you customers every day.",
  };
}
...
export default defineEventHandler(async (event) => {
  ...
  const analysedAt = new Date().toLocaleString("en-PK", {
